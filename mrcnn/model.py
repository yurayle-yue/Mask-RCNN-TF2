...
class DataGenerator(KU.Sequence):
    """An iterable that returns images and corresponding target class ids,
        bounding box deltas, and masks. It inherits from tensorflow.keras.utils.Sequence to avoid data redundancy
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            # 원본 tf1 버전 코드
            # anchors = tf.tensorflow.keras.layers.Lambda(lambda x: tf.Variable(anchors), name="anchors")(input_image)
            """케라스에서 상수 레이어에 해당하는 레이어가 없어서 람다 레이어를 사용하는 편법이 있었는데
            이제 통하지 않는다!
            그건 그렇고 인풋 함수가 있는데 왜 저렇게 짠거지??? 아무튼 커스텀 레이어 작성해서 상수 텐서를 리턴해주기로 함.
            """

            class ConstLayer(tf.tensorflow.keras.layers.Layer):
